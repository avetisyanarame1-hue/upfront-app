...
function readJsonBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', (chunk) => {
      ...
      if (size > MAX_BYTES) {
        rejected = true;
        // Do NOT destroy the socket — that drops the connection before a
        // response can be sent, which shows up to the client as a
        // confusing network error / 502 instead of a clear message.
        // Just stop buffering and drain the rest of the request quietly.
        req.removeAllListeners('data');
        req.resume();
        reject(new Error('That photo is too large. Please choose a smaller image (under ~6MB).'));
        return;
      }
      data += chunk;
    });
